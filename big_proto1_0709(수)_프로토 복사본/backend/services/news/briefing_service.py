...
class BriefingService:
    ...
    async def _call_bedrock_api(self, prompt: str) -> Dict[str, Any]:
        """AWS Bedrock API를 호출하여 실제 요약을 생성합니다."""
        
        try:
            self.logger.info("Bedrock API 호출 시작")
            ...
            response = await self.bedrock_client.generate(
                ...
                system_prompt="당신은 서울경제신문의 AI 뉴스 분석가입니다. 주어진 기사를 바탕으로 MZ세대를 위한 간결하고 정확한 FAQ를 생성합니다. 반드시 JSON 형식으로 응답해야 합니다.",
            ...
            self.logger.info(f"Bedrock API 응답 수신: {len(content)} 글자")
            ...
            try:
                ...
                self.logger.warning(f"Bedrock 응답을 JSON으로 파싱할 수 없음: {str(e)}, 텍스트 파싱 시도")
                return self._parse_text_response(content)
                
        except Exception as e:
            self.logger.error(f"Bedrock API 호출 오류: {str(e)}")
            raise
